...
@pytest.mark.parametrize('window, baseline', [(5, 10)])
def test_fixation_threshold_handler(fixation_fixture, window, baseline):
    """ Test that the handler uses data points for baseline and threshold
    until it has received enough, and the use them for measurement """
    handler = ThresholdDataHandler(
        measurement_func=lambda initTime, endTime, fx, fy, short_threshold, long_threshold: 1,
        subscribed_to=["initTime", "endTime", "fx", "fy"],
        window_length=window,
        window_step=window,
        baseline_length=baseline,
        threshold_length=baseline
    )

    for i in range(baseline):
        assert handler.baseline == 0
        assert handler.phase_func == handler.threshold_phase
        assert handler.short_threshold is None
        assert handler.long_threshold is None
        handler.add_data_point(fixation_fixture(i))

    for i in range(window*baseline):
        assert handler.baseline == 0
        assert handler.phase_func == handler._baseline_phase
        assert handler.short_threshold != 0
        assert handler.long_threshold != 0
        handler.add_data_point(fixation_fixture(i))

    assert handler.baseline != 0
    assert handler.phase_func == handler._csv_phase
